...
#' @param i.type.curve,i.type.threshold,i.type.intensity MEM configuration values.
...
#' @param mincases.pos minimum number of cases to end a seasonal alert. Default: 5
...
#' mun_list <- getCidades(uf = "Maranhão", datasource=connection)$municipio_geocodigo
#' thresC <- infodengue_apply_mem(mun_list, cid10 = "A92.0", database=connection)
...
infodengue_apply_mem <- function(mun_list, start_year=2010, end_year=as.integer(format(Sys.Date(), '%Y'))-1,
  ...
  if (!requireNamespace("mem", quietly = TRUE)) {
    stop("infodengue_apply_mem requires the optional package 'mem'.", call. = FALSE)
  }
  if (!requireNamespace("data.table", quietly = TRUE)) {
    stop("infodengue_apply_mem requires the optional package 'data.table'.", call. = FALSE)
  }
  .db_validate_connection(database)
  ...
  df.pop <- .repo_municipalities(database, cities = mun_list,
                                 columns = c("geocodigo", "populacao"))
  ...
  thresholds.table <- data.table::data.table(NULL)

   # Run by chuncks:
  for (mun_chunck in mun_list){
        print(mun_chunck)
    # Read historical cases table
    df.inc <- read.cases(start_year, end_year, cid10 = cid10,
                         datasource = database, mun_list=mun_chunck)
    effec_start_year <- min(floor(df.inc$SE/100))
    # Build incidence
    ...
    # Apply mem method
    thresholds.tab <- data.table::data.table(municipio_geocodigo=mun_chunck)
    ...
    thresholds.table <- data.table::rbindlist(list(thresholds.table, thresholds.tab))
...
#' @param end_year Historical data final year to consider. Default: previous year from Sys.Date()
#' @param mun_list Vector of municipalities geocode. Default: NULL (i.e., all municipalites)
#' @param nome Name assigned to the aggregation.
#' @param database DBI connection.
#' @param passwd database password for writing output to db if write='db'.
#' @param cid10 cid 10 code. Dengue = "A90" (default), Chik = "A92.0", Zika = "A92.8"
#' @param i.n.max Number of points by seasons to be used for pre-epidemic and epidemic regions to calculate each threshold.
...
#' mun_list <- c(4212650, 4209102,4216503,4214607,4212502,4218905,4212601,4214805,
#' 4212650,4217006,4212700,4214706,4213104,4200804)
#' mun_list <- getCidades(uf = "Rio de Janeiro", regional = "Metropolitana I", datasource=connection)
...
infodengue_apply_mem_agreg <- function(mun_list,
  ...
  if (!requireNamespace("mem", quietly = TRUE)) {
    stop("infodengue_apply_mem_agreg requires the optional package 'mem'.", call. = FALSE)
  }
  if (!requireNamespace("data.table", quietly = TRUE)) {
    stop("infodengue_apply_mem_agreg requires the optional package 'data.table'.", call. = FALSE)
  }
  .db_validate_connection(database)
  ...
  names(df.pop)[1] <- "municipio_geocodigo"
   ...
   df.inc <- read.cases(start_year, end_year,
                        cid10 = cid10, datasource = database, mun_list=mun_list)
   ...
   df.inc.a <- df.inc %>%
     ...
     summarise(casos = sum(casos)) %>%
         mutate(municipio_geocodigo = nome) # variavel nao usada, trocar depois pela regional

   df.inc.a['populacao'] <- pop
  ...
  # Build incidence
   ...
   df.inc.a['inc'] <- df.inc.a$casos * 100000.0 / df.inc.a['populacao']
   #df.inc['municipio_geocodigo'] <- nome

   # Store only necessary data, separating seasons by columns
   #dfsimple <- df.inc[df.inc$SE > max(df.inc$SE[df.inc$SE<(effec_start_year+1)*100])-12 &
   #                      df.inc$SE < (effec_start_year+1)*100+41, c('municipio_geocodigo', 'SE', 'inc')]
   dfsimple <- df.inc.a[df.inc.a$SE > max(df.inc.a$SE[df.inc.a$SE<(effec_start_year+1)*100])-12 &
                         df.inc.a$SE < (effec_start_year+1)*100+41, c("municipio_geocodigo",'SE', 'inc')]
    ...
    for (i in (effec_start_year+1):end_year){
      if (max(df.inc$SE) >= (i+1)*100 + 41){
        dfsimple <- bindseason(df.inc.a, dfsimple, i)
        seasons <- cbind(seasons, paste0(i,'-',i+1))
      }
    }

    # Apply quantile method (new)
    quantile.tab <- df.inc.a %>%
      summarise(quant_pre = max(mincases.pre, quantile(casos, probs = 0.10), na.rm = TRUE)/mean(populacao)*1e5,
                quant_pos = max(mincases.pre,quantile(casos, probs = 0.10), na.rm = TRUE)/mean(populacao)*1e5,
                quant_epidemico = max(mincases.epi,quantile(casos, probs = limiar.epidemico), na.rm = TRUE)/mean(populacao)*1e5)
...
mem_curve <- function(mun_list,
      ...
      if (!requireNamespace("mem", quietly = TRUE)) {
            stop("mem_curve requires the optional package 'mem'.", call. = FALSE)
      }
      if (!requireNamespace("data.table", quietly = TRUE)) {
            stop("mem_curve requires the optional package 'data.table'.", call. = FALSE)
      }
      .db_validate_connection(database)
      ...
      curveMEM <- fit.mem(dfsimple, seasons, i.n.max=i.n.max, i.level.threshold=limiar.preseason,
                                    i.level.intensity=limiar.epidemico,
                                    i.type.curve=i.type.curve, i.type.threshold=i.type.threshold,
