...
#'@param datasource Database connection used by legacy adjustment methods.
#'@param seed Optional seed for stochastic Bayesian nowcasting.
...
#'@param ... Additional arguments passed to the selected adjustment method.
...
#'interation = TRUE, datasource = connection)
#'t2 <- Sys.time()
...
#'nowSE = 202414,datasource = connection)
#'t2 <- Sys.time()

.nowcast_cases_impl <- function(obj, datas = NULL, method = "none",
                            pdig = plnorm((1:20)*7, 2.5016, 1.1013),
                            Dmax = 10, nyears = 2, datasource = NULL, nowSE,
                            safelimit = 5, seed = NULL, workers = 1L,
                            verbose = FALSE, ...) {
  workers <- .validate_workers(workers)
  if (!method %in% c("none", "bayesian")) {
    stop("adjustIncidence: method must be 'none' or 'bayesian'.", call. = FALSE)
  }
  inform <- function(...) if (isTRUE(verbose)) message(...)
  ...
  if(sum(tail(obj$casos, n = 52), na.rm = TRUE) <= 50 |
    ...
    inform("less than 50 cases in the last 12 months or less than 5 cases in the last month. Nowcasting not done")
    return(obj)
  }

 if (method == "bayesian"){
       inform(paste("computing nowcasting for city", city, "date", nowSE, "..."))
       if (is.null(datas)) {
             stop("adjustIncidence: Bayesian nowcasting requires in-memory `datas`.",
                  call. = FALSE)
       }
       ...
       resfit <- bayesnowcasting(dados, Dmax = Dmax, nowSE = nowSE,
                                 seed = seed, workers = workers, verbose = verbose)

       # checking estimates
       if(!is.null(resfit)){
         if(tail(resfit$Median, n = 1) >
            safelimit * sum(tail(obj$casos, n = 5),na.rm = TRUE)){
           inform("nowcast estimate is too large, returning the original count.")
         } else {
           if(tail(resfit$LS, n = 1) >
              10 * safelimit * sum(tail(obj$casos, n = 5),na.rm = TRUE)){
             inform("upper limit nowcasting too large, returning t_cases_max = NA")
             resfit$LS <- NA
           expl <- which(resfit$LS > 10000)  # check if any LS was too big
           resfit$LS[expl] <- NA
           inform("upper limit nowcasting too large in some dates, returning t_cases_max = NA")
           }
           inform("bayesnowcasting done")
       ...
       } else {inform("nowcasting failed, returning the original count")}
 }

  if(method=="none") inform("nowcasting not done, returning the original counts")
...
#'@title Correct incidence data with notification delay (nowcasting).
#'@export
#'@param d data.frame with individual cases, containing columns municipio_geocodigo, dt_notific, dt_sin_pri, dt_digita
...
#'@param seed Optional seed for posterior and predictive sampling.
#'@param workers Positive number of workers used by INLA.
#'@param verbose Whether to emit progress messages.
...
#'lastday = as.Date("2024-03-23"), datasource=connection)  # Not run without connection
#'resfitcIsT<-bayesnowcasting(dados, nowSE = 202412)

bayesnowcasting <- function(d, Dmax = 10, nowSE, nweeks = 50,
                            interacao = FALSE, tweet = FALSE, seed = NULL,
  ...
  # check input
  if(is.null(names(d))) {
    inform("bayesnowcasting: no data, returning NULL")
    return(NULL)}

  if(nrow(d) < 50) {
    inform("bayesnowcasting: few data, returning NULL")
    return(NULL)}
  ...
  if(length(wrongdates) > 0) {
    inform(paste(length(wrongdates), "registros com datas de inicio de sintomas invalidas"))
    d <- d[-wrongdates,]
  }
  ...
  # checking again
  if(is.null(names(d))) {
    inform("bayesnowcasting: no valid data, returning NULL")
    return(NULL)}

  if(nrow(d) < 50) {
    inform("bayesnowcasting: few valid data, returning NULL")
    return(NULL)}

  inform(paste("nowcast will be calibrated with", nrow(d), "cases"))
  ...
  # nowcasting
  d <- d %>% mutate(
    dt_sinpri_epiweek = lubridate::epiweek(dt_sin_pri),  # se da data sinpri
    ...
    dt_sinpri_epiyear = lubridate::epiyear(dt_sin_pri),
    dt_sinpri_SE = dt_sinpri_epiyear*100 + dt_sinpri_epiweek,
    dt_digita_epiweek = lubridate::epiweek(dt_digita),
    dt_digita_epiyear = lubridate::epiyear(dt_digita),
  ...
  pred.dengue <- nowcasting(output.dengue, dados.ag,
                            Dm = Dmax, Fim = Fim, seed = seed) # max(dados.ag$Date))
...
getdelaydata <- function(cities, nyears= 2, cid10 = "A90", lastday = Sys.Date(),
  ...
  assert_that(length(nyears) == 1L && is.numeric(nyears) && nyears > 0,
              msg = "getdelaydata: nyears must be positive")
  lastday <- as.Date(lastday)
  firstday <- lastday - nyears * 365
  dd <- .repo_notifications(
    datasource, cities, disease$values, firstday, lastday,
    columns = c("municipio_geocodigo", "dt_notific", "dt_sin_pri", "dt_digita")
  )


  if(nrow(dd) == 0){
    message(paste("getdelaydata: found no", disease$canonical, "data for",
