...
#'@param obj A data frame containing at least `SE` and the count column.
#'@param count Name of the case-count column in `obj`.
#'@param gtdist Generation-time distribution, either `"normal"` or `"delta"`.
...
#'@param a0,b0 Parameters of the beta prior.
#'@noRd
...
.estimate_rt_impl <- function(obj, count = "casos", gtdist, meangt, sdgt, CI = "beta", alpha = .95, a0 = 2 , b0 = 3){
...
#' @param workers Positive number of parallel workers; defaults to one.
#' @param nc Deprecated alias for `workers`.
...
#' @examplesIf FALSE
#'cli = getWU(stations = 'SBGL', vars=c("temp_med"), datasource=connection) %>%
...
GenTimeDist <- function(serTemp, cid10 = "A90", GT.max = 10,
                        smooth = "sinusoidal", workers = 1L, nc = workers){

      if (!missing(nc)) workers <- nc
      workers <- .validate_workers(workers)
      if (!is.numeric(serTemp) || !length(serTemp) || any(!is.finite(serTemp))) {
            stop("`serTemp` must be a non-empty finite numeric vector.", call. = FALSE)
      }
      if (!smooth %in% c("sinusoidal", "raw")) {
            stop("`smooth` must be 'sinusoidal' or 'raw'.", call. = FALSE)
      }
      ...
      # for all weeks...
      gt <- if (workers == 1L || .Platform$OS.type == "windows") {
            vapply(tt, evalGenTimeDist, numeric(GT.max + 1L))
      } else {
            parallel::mcmapply(evalGenTimeDist, tt, mc.cores = workers,
                               mc.preschedule = TRUE)
      }
...
applyGenTimeDist <- function(cities, stations, cid10 = "A90", datasource,...){
      ...
      if(missing(stations) & !missing(cities)){
            ...
            city_table <- .repo_regional_health(datasource, cities)
